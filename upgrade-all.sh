#!/usr/bin/env bash
#
# Harness Workflow - 批量升级脚本
#
# 读取 ~/.claude/harness-projects.yaml，对所有 type: harness-workflow
# 的项目依次执行本仓库的 upgrade.sh。
#
# 用法：
#   cd /path/to/harness-workflow
...
#   - Git 未提交改动的项目由本项目的 upgrade.sh 自己询问
#
set -u
...
EXPECTED_TYPE="harness-workflow"
...
echo -e "${GREEN}=== Harness Workflow 批量升级 ===${NC}"
echo ""
...
# ----- 解析 yaml（与 project-harness-template 版本一致） -----
NAMES=()
PATHS=()
TYPES=()
...
if [ -n "$PASS_SAFE" ]; then
  echo -e "${YELLOW}模式：--safe（改过的命令文件只生成 .new）${NC}"
else
...
${BLUE}下一步建议：${NC}
  ...
  2. 各项目里 git commit -m "harness: batch upgrade to 2026-04"
EOF
