...
├── INSTALL.md                # 本文件
├── README.md
├── CLAUDE.md                 # 项目仓库叠加模板（覆盖到已有项目根目录）
├── HARNESS_PHILOSOPHY.md     # 设计哲学（建议通读）
├── .mcp.json                 # 项目级 MCP 配置模板
├── setup.sh                  # 一键安装脚本（项目仓库 + 全局命令）
├── upgrade.sh                # 已接入项目的升级脚本
├── upgrade-all.sh            # 批量升级所有注册项目
├── .claude/
│   ├── commands/             # 项目级命令（13 个，含 /adversarial-review、/metrics、/run-tasks、/command-feedback）
│   │   ├── init-baseline.md
│   │   ├── iterate.md
│   │   ├── design.md
│   │   ├── impl.md
...
│   │   ├── spec-feedback.md
│   │   └── command-feedback.md
│   └── knowledge/            # 领域知识种子（/impl、/review 按需加载）
│       ├── backend/          # architecture.md / api-conventions.md / sxp-framework.md
│       ├── frontend/         # react-patterns.md / taro-patterns.md
│       ├── testing/          # standards.md
│       ├── collaboration.md  # 多 agent 并行协作规范
│       └── red-lines.md      # 质量红线
...
│   └── knowledge/
│       └── red-lines.md
├── dashboard/                # /dashboard 看板聚合脚本（装到 ~/.claude/harness-dashboard/）
│   └── build.py
...
### 项目仓库命令（`.claude/commands/`）
只在该项目内生效：
- `/impl` — **唯一任务入口**；AI 自动判定小/大任务，小任务走全自动路径，大任务转 `/iterate`
- `/iterate` — 迭代影响分析 + 任务清单生成
- `/design` — 生成详细设计
- `/run-tasks` — 批量循环执行 checklist 中的任务（支持 `--parallel N`）
- `/review` — 代码校验（Generator 自审）
- `/adversarial-review` — 独立 Evaluator 对抗式评估（PR/Sprint 合并前必须新开 session）
- `/test-gen` — 基于设计契约单独生成测试
- `/preflight` — 提交前全面检查
- `/metrics` — Harness 运行指标聚合
- `/init-baseline` — 旧项目首次接入
- `/record-session` — 会话记录
- `/spec-feedback` — 设计反馈
- `/command-feedback` — 记录对 slash 命令本身的修改建议
...
# 2. 运行安装脚本
bash /path/to/harness-workflow/setup.sh
...
# 把模板复制过来
cp -r /path/to/harness-workflow/consensus-hub/. .
...
cp -r /path/to/harness-workflow/ai-workflow/. .
mkdir -p eval/cases
...
# 复制 CLAUDE.md + HARNESS_PHILOSOPHY.md（注意：如果已有 CLAUDE.md，需手动合并）
cp /path/to/harness-workflow/CLAUDE.md ./
cp /path/to/harness-workflow/HARNESS_PHILOSOPHY.md ./

# 复制命令
mkdir -p .claude/commands
cp /path/to/harness-workflow/.claude/commands/*.md ./.claude/commands/

# 复制 knowledge 种子（backend / frontend / testing + collaboration.md + red-lines.md）
mkdir -p .claude/knowledge/backend .claude/knowledge/frontend .claude/knowledge/testing
cp -r /path/to/harness-workflow/.claude/knowledge/. ./.claude/knowledge/

# 复制 MCP 配置
cp /path/to/harness-workflow/.mcp.json ./
...
```bash
mkdir -p ~/.claude/commands
cp /path/to/harness-workflow/global-commands/*.md ~/.claude/commands/

# /dashboard 聚合脚本（跨项目看板数据源）
mkdir -p ~/.claude/harness-dashboard
cp /path/to/harness-workflow/dashboard/build.py ~/.claude/harness-dashboard/
...
Claude 会扫描代码库、检测技术栈、填充 `.claude/knowledge/` 模板、生成基线文档。
...
手动合并。把模板中的"你是谁"、"核心原则"、"可用命令"追加到已有 CLAUDE.md。项目特有的规则放到 `.claude/knowledge/` 下。

### Q2: 团队成员如何同步这套配置？
`.claude/commands/`、`.claude/knowledge/`、`.mcp.json`、`CLAUDE.md`、`HARNESS_PHILOSOPHY.md` 都应该 commit 到 Git，团队成员 pull 就有了。全局命令（`~/.claude/commands/`）每个人自己安装。
...
安装完后建议：
1. 跑通一次完整流程（`/impl` → `/adversarial-review` → `/metrics`）
2. 根据团队习惯调整 `.claude/knowledge/red-lines.md`
3. 在 `.claude/knowledge/backend/` 和 `.claude/knowledge/frontend/` 中补充团队积累的约定
