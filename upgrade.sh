#!/usr/bin/env bash
#
# Harness Workflow 升级脚本（2026-04 新增 /adversarial-review、/metrics、/run-tasks、tasks.yaml）
#
# 用法：
#   1. 进入已接入 harness 的项目根目录（必须已有 .claude/commands/ 和 CLAUDE.md）
#   2. 运行：bash /path/to/harness-workflow/upgrade.sh [--safe]
#
# 默认行为（直接覆盖 + 自动备份）：
#   - 新文件、新目录：直接添加（已存在则跳过）
...
echo -e "${GREEN}=== Harness Workflow 升级脚本 ===${NC}"
echo ""
...
if [ ! -d "./.claude/commands" ] || [ ! -f "./CLAUDE.md" ]; then
  echo -e "${RED}错误：看起来这个项目还没接入 harness${NC}"
...
# ----- Step 2: 命令文件 -----
# 新增命令：adversarial-review.md, metrics.md, run-tasks.md → copy_if_missing
...
for cmd_file in "$TEMPLATES_DIR/project/.claude/commands/"*.md; do
  ...
  case "$name" in
    adversarial-review.md|metrics.md|run-tasks.md|command-feedback.md)
    ...
    impl.md)
      ...
      ;;
    *)
      SKIPPED+=(".claude/commands/$name（不在本次升级范围，保留原文件）")
      ;;
  esac
done
...
# ----- Step 3.5: 安装 /dashboard 看板 + 注册本项目 -----
echo ""
...
# 全局 /dashboard 命令
if [ -f "$TEMPLATES_DIR/global-commands/dashboard.md" ]; then
  mkdir -p "$HOME/.claude/commands"
  if [ ! -f "$HOME/.claude/commands/dashboard.md" ] || ! cmp -s "$TEMPLATES_DIR/global-commands/dashboard.md" "$HOME/.claude/commands/dashboard.md"; then
    cp "$TEMPLATES_DIR/global-commands/dashboard.md" "$HOME/.claude/commands/dashboard.md"
    ADDED+=("~/.claude/commands/dashboard.md")
  else
    SKIPPED+=("~/.claude/commands/dashboard.md（内容相同）")
  fi
fi
...
${BLUE}全局：${NC}
  本脚本已经为你装好 /dashboard 和聚合脚本，并把本项目注册进 ~/.claude/harness-projects.yaml。
  其他项目：各自跑一次 upgrade.sh，注册表会自动累积。
  ~/.claude/config.yaml 不会被动，避免影响已有全局配置。

${BLUE}验证安装：${NC}
  在项目里打开 Claude Code：
  - 项目命令：/adversarial-review、/metrics、/run-tasks
  - 全局命令：/dashboard --open （一屏看所有注册项目的指标 + 对抗评估 + 时间线）
EOF
